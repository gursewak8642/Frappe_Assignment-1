...
def create_record():
    ...
    try:
        ...
        doc.insert()
        frappe.db.commit()  # Commit transaction to save the new record
        return doc.as_dict()
    except Exception as e:
...
def update_record():
    ...
    try:
        ...
        doc.save()
        frappe.db.commit()  # Commit transaction to save changes
        return doc.as_dict()
    except Exception as e:
        frappe.db.rollback()  # Rollback in case of error
...
def delete_specific_field():
    ...
    try:
        ...
        if hasattr(doc, fieldname):
            setattr(doc, fieldname, None)  # Set the field to None
            doc.save()
            frappe.db.commit()  # Commit transaction to save changes
